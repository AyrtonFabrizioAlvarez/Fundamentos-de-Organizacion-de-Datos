...
begin
	...
	writeln('7: Eliminar un elemento "x" escribirendo en su lugar el ultimo elemento del archivo y truncando para achicar su tamaño ');
...
procedure hacerBaja(var arch:archivo);
	...
	procedure eliminar(var arch:archivo ; buscado:String ; var ok:boolean);
	var
		reg, ultimo:empleado;
		pos:integer;
	begin
		...
		begin
			...
			begin
				pos:= filePos(arch)-1;			//GUARDO LA POSICION ANTERIOR (ELEMENTO A ELIMINAR)
				seek(arch, FileSize(arch) - 1);	//MUEVO EL PUNTERO AL ULTIMO ELEMENTO
				read(arch, ultimo);				//ME GUARDO EL ULTIMO ELEMENTO
				seek(arch, pos);				//VOY A LA POSICION DEL ELEMENTO A ELIMINAR
				write(arch, ultimo);			//SOBREESCRIBO EL ELEMENTO POR EL ULTIMO	
				seek(arch, fileSize(arch)-1);	//VUELVO A POSICIONARME EN EL ULTIMO ELEMENTO
				Truncate(arch);					//REALIZO EL TRUNCAMIENTO PARA CAMBIAR LA MARCA DE EOF
				ok:= true;						//VARIABLES PARA SABER SI LA BAJA FUE EXITOSA
			end
			else								//SI NO LO ENCONTRE
				leer(arch, reg);				//SIGO LEYENDO
		end;
	end;
...
begin
	ok:= false;
	writeln('Ingrese el nombre del empleado al que desea dar de baja');
	readln(buscado);
	eliminar(arch, buscado, ok);
	if (ok) then
		writeln('El empleado con nombre ', buscado, ' se elimino correctamente')
	else
		writeln('El empleado con nombre ', buscado, ' no se encuentra en el archivo');
end;
...
BEGIN
	...
	begin
		...
		case opcion1 of
			...
			'7':begin	//ELIMINAR UNA POSICION SOBREESCRIBIENDO CON EL ULTIMO ELEMENTO DEL ARCHIVO PARA LUEGO TRUNCAR
					writeln('Ingrese el nombre del archivo que desea abrir para realizar una baja');
					readln(archFisico);
					assign(archLogico, archFisico);
					reset(archLogico);
					hacerBaja(archLogico);
					close(archLogico);
				end;
			else
				ejecutar:= false;
